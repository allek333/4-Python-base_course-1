...
my_sort_list = {number_list[el] for el in range(1, len(number_list)) if number_list(el+1) > number_list(el)}
# my_sort_list = {el for el in number_list if number_list(el+1) > number_list(el)}
'''TypeError: 'list' object is not callable'''
# my_sort_list = {el f-or el in enumerate(number_list) if [el+1] > [el]}
